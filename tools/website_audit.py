...
def curl(url, method="GET", head_only=False, accept_encoding="gzip, br, deflate",
    ...
    hdr_file = "/tmp/.audit_hdr"
    body_file = "/tmp/.audit_body" if want_body else os.devnull
...
STACK_SIGNATURES = [
    ...
    (r"wp-content/plugins/contact-form-7|wpcf7", "Contact Form 7 (Formular-Plugin)"),
    (r"wp-content/plugins/wpforms|wpforms-", "WPForms (Formular-Plugin)"),
    (r"gravityforms|gform_", "Gravity Forms (Formular-Plugin)"),
    (r"ninja-forms|nf-form", "Ninja Forms (Formular-Plugin)"),
    (r"formidable|frm_forms", "Formidable Forms (Formular-Plugin)"),
    (r"forminator", "Forminator (Formular-Plugin)"),
    (r"fluentform|fluent-form", "Fluent Forms (Formular-Plugin)"),
    (r"elementor", "Elementor (Page Builder)"),
    (r"/divi/|et_pb_", "Divi (Theme/Builder)"),
    (r"wpbakery|js_composer|vc_row", "WPBakery (Page Builder)"),
...
def measure(url, runs=RUNS):
    ...
    meta = {}
    for i in range(runs):
        stats, headers, _ = curl(url, want_headers=(i == 0))
        if "error" in stats:
...
def asset_inventory(base_url, doc, limit=80):
    ...
    for kind, raw in urls:
        ...
        if "error" in stats:
            results.append({"kind": kind, "url": absu, "error": stats["error"]})
            continue
...
def tls_info(hostname):
    ...
    try:
        ...
        return {"handshake": brief.strip()[:800], "cert": cert.stdout.strip()[:800]}
...
def build_report(base, perf_pages, form_pages, seo):
    ...
    for label, m in perf_pages:
        ...
        if enc:
            ...
            if enc.get("bytes_uncompressed") and enc.get("bytes_compressed"):
                try:
                    ...
                    pass
            o.append(f"**Kompression:** `content-encoding: {enc.get('encoding_header') or 'KEINE'}` - "
        ...
        if not enc.get("encoding_header"):
            ...
            finding(o, f"Kompression aktiv: `{enc['encoding_header']}`",
...
def load_page(url):
    ...
    forms = [analyse_form(f, final, body) for f in doc.forms if f.get("fields")]
    return {
        "url": final,
...
def main():
    ...
    if home.get("error"):
        ...
        doc = home["doc"]
        imgs = doc.images
        stack = detect(STACK_SIGNATURES, home["html"])
        ...
        seo = {
            ...
            "stack": stack,
            "wp_plugins": wp_plugins(home["html"]),
